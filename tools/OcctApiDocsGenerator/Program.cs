...
internal static partial class Program
{
    ...
    public static int Main(string[] args)
    {
        try
        {
            ...
            var assemblyPaths = inputs
                .SelectMany(input => Directory.EnumerateFiles(input.OutputDirectory, "*.dll", SearchOption.TopDirectoryOnly))
    ...
    private static AssemblyInput[] DiscoverAssemblies(string root, string configuration, string targetFramework)
    {
        ...
        return candidates
            .Where(candidate => File.Exists(Path.Combine(root, candidate.Directory.Replace('/', Path.DirectorySeparatorChar), candidate.Name + ".csproj")))
            .Select(candidate =>
            {
    ...
    private static string RenderType(string assemblyName, Type type, XmlDocs docs, bool chinese)
    {
        ...
        var constructors = type.GetConstructors(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).Cast<MethodBase>();
        ...
        var methods = type.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly).Where(method => !method.IsSpecialName).Cast<MethodBase>();
        var fields = type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly).Where(field => type.IsEnum ? field.IsLiteral : !field.IsSpecialName);

        RenderMethods(builder, type, constructors, docs, chinese ? "构造函数" : "Constructors", chinese);
        RenderProperties(builder, type, properties, docs, chinese);
        RenderEvents(builder, type, events, docs, chinese);
        RenderMethods(builder, type, methods, docs, chinese ? "方法" : "Methods", chinese);
        RenderFields(builder, type, fields, docs, chinese);
    ...
    private static void RenderFields(StringBuilder builder, Type type, IEnumerable<FieldInfo> fields, XmlDocs docs, bool chinese)
    {
        ...
        foreach (var value in values)
        {
            var literal = value.IsLiteral ? " = " + Convert.ToString(value.GetRawConstantValue(), System.Globalization.CultureInfo.InvariantCulture) : string.Empty;
            var summary = docs.Field(type, value);
            builder.AppendLine($"- `{value.Name}` — `{DisplayType(value.FieldType)}`{literal}{(string.IsNullOrWhiteSpace(summary) ? string.Empty : " — " + summary)}");
        }
        builder.AppendLine();
    }
    ...
    private static string DisplayType(Type type)
    {
        ...
        var aliases = new Dictionary<Type, string>
        {
            ...
            [typeof(float)] = "float", [typeof(double)] = "double", [typeof(string)] = "string", [typeof(object)] = "object"
        };
