...
main 发布流程会生成中英文完整 API Reference，执行 Release Native/Managed Build 与 Native Smoke，生成 Binary SDK，再通过临时 worktree 同步 `dist/win-x64` 到 demo。Managed regression tests 保留在显式测试流程中，不重复阻塞 Binary SDK 发布。
