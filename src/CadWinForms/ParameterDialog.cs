...
internal sealed class ParameterDialog : Form
{
    ...
    private ParameterDialog(string title, IReadOnlyList<CadParameterDefinition> parameters)
    {
        ...
        var ok = new Button { Text = CadLocalization.Text("Dialog.Ok"), DialogResult = DialogResult.OK, AutoSize = true, Padding = new Padding(12, 2, 12, 2) };
        var cancel = new Button { Text = CadLocalization.Text("Dialog.Cancel"), DialogResult = DialogResult.Cancel, AutoSize = true, Padding = new Padding(12, 2, 12, 2) };
    ...
    public static bool TryGetValues(IWin32Window owner, string title, IReadOnlyList<CadParameterDefinition> parameters, out IReadOnlyDictionary<string, string> values)
    {
    ...
    private static Control CreateEditor(CadParameterDefinition parameter)
    {
        ...
        {
            CadParameterKind.Boolean => new CheckBox { Checked = string.Equals(parameter.DefaultValue, "true", StringComparison.OrdinalIgnoreCase), AutoSize = true },
