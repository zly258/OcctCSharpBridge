...
| 02 | [架构与边界](02_架构与边界.md) | Native/C# 分层、`main`/`demo` 边界、Owner-aware、生命周期、手工同步规则 |
...
└─ build.ps1                    本地 validate/build/test/pack/smoke 入口
```
...
- `demo/docs`：只维护 Demo 构建、运行、发布、共享层同步等应用层说明。
- `demo` 不复制 `main` 的 API Coverage、B-Spline、Topology、Runtime 等 Bridge 技术文档。
- Bridge 能力发生变化时先更新 `main` 的代码和文档，再按共享层规则同步代码到 `demo`。

这样避免同一能力存在多份 `.md` / `.zh-CN.md` 文档并逐渐不一致。
