...
[[ "$(uname -s)" == "Linux" ]] || fail "publish.sh must run on Linux."
case "$(uname -m)" in x86_64|amd64) ;; *) fail "Only Linux x64 is currently supported." ;; esac
...
NATIVE_SOURCE="${ROOT_DIR}/build/native/bin/${CONFIGURATION}/libOcctNative.so"
...
[[ -f "${CONTRACT}" ]] || fail "bridge-contract.json was not found."

log "Building native bridge and Avalonia demo (${CONFIGURATION})..."
"${ROOT_DIR}/build.sh" "${CONFIGURATION}"
[[ -f "${NATIVE_SOURCE}" ]] || fail "Native bridge was not produced: ${NATIVE_SOURCE}"
...
# Bundle OCCT and non-system third-party libraries while leaving glibc, X11 and the graphics stack to the target OS.
declare -A queued=()
declare -A copied=()
...
should_bundle_dependency() {
    ...
    [[ "${path}" == "${OCCT_ROOT}"/* ]] && return 0
    [[ "${path}" == "${OCCT_LIB_DIR}"/* ]] && return 0
    return 1
}
...
# Validate the packaged native bridge against the package-local dependency closure.
...

{
    ...
    echo "OCCT source root: ${OCCT_ROOT}"
    echo
    echo "SHA256:"
