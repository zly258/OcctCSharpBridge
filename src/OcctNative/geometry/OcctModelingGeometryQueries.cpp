...
namespace
{
    double edgeLength(BRepAdaptor_Curve& curve)
    {
        return GCPnts_AbscissaPoint::Length(curve, curve.FirstParameter(), curve.LastParameter());
    }

    double edgeParameterAtLength(BRepAdaptor_Curve& curve, double totalLength, double length)
    {
        if (!std::isfinite(length) || length < 0.0)
            throw std::invalid_argument("Arc length must be finite and non-negative.");
        if (length > totalLength + Precision::Confusion())
            throw std::invalid_argument("Arc length exceeds the edge length.");
        if (totalLength <= Precision::Confusion() || length <= Precision::Confusion())
...
extern "C"
{
    OcctStatus occt_model_vertex_point(OcctModelingSessionHandle handle, OcctObjectId vertexId, OcctPoint3d* result)
    {
    ...
    OcctStatus occt_model_edge_endpoints(OcctModelingSessionHandle handle, OcctObjectId edgeId, OcctPoint3d* start, OcctPoint3d* end)
    {
        ModelSession* model = sessionOf(handle);
        if (model == nullptr) return OcctStatus_ErrorInvalidHandle;
    ...
    OcctStatus occt_model_edge_point_at(OcctModelingSessionHandle handle, OcctObjectId edgeId, double normalizedParameter, OcctPoint3d* resultPoint, OcctVector3d* resultTangent)
    {
        ModelSession* model = sessionOf(handle);
        if (model == nullptr) return OcctStatus_ErrorInvalidHandle;
    ...
    OcctStatus occt_model_edge_length(OcctModelingSessionHandle handle, OcctObjectId edgeId, double* result)
    {
        ModelSession* model = sessionOf(handle);
        if (model == nullptr) return OcctStatus_ErrorInvalidHandle;
        if (result == nullptr) return OcctStatus_ErrorInvalidArgument;
    ...
    OcctStatus occt_model_edge_point_at_length(OcctModelingSessionHandle handle, OcctObjectId edgeId, double length, double* curveParameter, OcctPoint3d* resultPoint, OcctVector3d* resultTangent)
    {
        ModelSession* model = sessionOf(handle);
        if (model == nullptr) return OcctStatus_ErrorInvalidHandle;
        if (curveParameter == nullptr || resultPoint == nullptr || resultTangent == nullptr) return OcctStatus_ErrorInvalidArgument;
        *curveParameter = 0.0;
        *resultPoint = {};
        *resultTangent = {};
        return executeStatus(model, [&]
        {
            ...
            if (shape.ShapeType() != TopAbs_EDGE) throw std::invalid_argument("Input must be an edge.");
            BRepAdaptor_Curve curve(TopoDS::Edge(shape));
            const double parameter = edgeParameterAtLength(curve, edgeLength(curve), length);
            gp_Pnt point;
            gp_Vec tangent;
            curve.D1(parameter, point, tangent);
            if (tangent.SquareMagnitude() <= Precision::SquareConfusion()) throw std::runtime_error("Edge tangent is undefined at the requested arc length.");
            tangent.Normalize();
    ...
    OcctStatus occt_model_edge_sample_points_snapshot_get(
        ...
        int* required)
    {
        ModelSession* model = sessionOf(handle);
        if (model == nullptr) return OcctStatus_ErrorInvalidHandle;
        if (sampleCount < 2 || capacity < 0 || required == nullptr) return OcctStatus_ErrorInvalidArgument;
        *required = sampleCount;
        if (results == nullptr && capacity != 0) return OcctStatus_ErrorInvalidArgument;
        if (results != nullptr && capacity < sampleCount) return OcctStatus_ErrorBufferTooSmall;
        ...
        return executeStatus(model, [&]
        {
            ...
            BRepAdaptor_Curve curve(TopoDS::Edge(shape));
            const double totalLength = edgeLength(curve);
            for (int index = 0; index < sampleCount; ++index)
            {
                const double length = totalLength * static_cast<double>(index) / static_cast<double>(sampleCount - 1);
                const gp_Pnt point = curve.Value(edgeParameterAtLength(curve, totalLength, length));
                results[index] = {point.X(), point.Y(), point.Z()};
            }
        });
    }

    OcctStatus occt_model_edge_curve_type(OcctModelingSessionHandle handle, OcctObjectId edgeId, OcctCurveType* result)
    {
        ModelSession* model = sessionOf(handle);
        if (model == nullptr) return OcctStatus_ErrorInvalidHandle;
        if (result == nullptr) return OcctStatus_ErrorInvalidArgument;
    ...
    OcctStatus occt_model_face_surface_type(OcctModelingSessionHandle handle, OcctObjectId faceId, OcctSurfaceType* result)
    {
        ModelSession* model = sessionOf(handle);
        if (model == nullptr) return OcctStatus_ErrorInvalidHandle;
        if (result == nullptr) return OcctStatus_ErrorInvalidArgument;
        *result = OcctSurface_Other;
        return executeStatus(model, [&]
        {
            ...
            *result = static_cast<OcctSurfaceType>(toOcctSurfaceType(BRepAdaptor_Surface(TopoDS::Face(shape), Standard_True).GetType()));
        });
    }

    OcctStatus occt_model_face_uv_bounds(OcctModelingSessionHandle handle, OcctObjectId faceId, OcctUvBounds* result)
    {
        ModelSession* model = sessionOf(handle);
        if (model == nullptr) return OcctStatus_ErrorInvalidHandle;
        if (result == nullptr) return OcctStatus_ErrorInvalidArgument;
    ...
    OcctStatus occt_model_face_point_normal(OcctModelingSessionHandle handle, OcctObjectId faceId, double u, double v, OcctPoint3d* resultPoint, OcctVector3d* resultNormal)
    {
        ModelSession* model = sessionOf(handle);
        if (model == nullptr) return OcctStatus_ErrorInvalidHandle;
