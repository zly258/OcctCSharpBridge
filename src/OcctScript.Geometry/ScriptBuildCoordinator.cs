...
public enum CommandBuildStatus { Succeeded, Failed, Skipped }

public sealed record CommandBuildState(Guid CommandId, CommandBuildStatus Status, OcctModelShape Shape, IReadOnlyList<ShapeBuildMessage> Messages)
{
    public bool Success => Status != CommandBuildStatus.Failed;
}

public sealed record DocumentBuildResult(bool Success, IReadOnlyDictionary<Guid, OcctModelShape> Shapes, IReadOnlyList<CommandBuildState> Commands, TimeSpan Duration);
...
public sealed class ScriptBuildCoordinator : IDisposable
{
    ...
    public ScriptBuildCoordinator(CommandRegistry? commandDefinitions = null, CommandShapeBuilderRegistry? builders = null, ExpressionEngine? expressionEngine = null)
    {
    ...
    public OcctModelingSession Session => disposed ? throw new ObjectDisposedException(nameof(ScriptBuildCoordinator)) : session;

    public DocumentBuildResult Build(ScriptDocument document, IReadOnlyDictionary<string, double> parameters)
    {
        ...
        foreach (var pair in plan.Errors) states[pair.Key] = new CommandBuildState(pair.Key, CommandBuildStatus.Failed, default, pair.Value);
        ...
        foreach (var command in plan.OrderedCommands)
        {
            ...
            {
                states[command.Id] = new CommandBuildState(command.Id, CommandBuildStatus.Skipped, default, [ShapeBuildMessage.Warning("COMMAND_DISABLED", $"Command '{command.Name}' is disabled.")]);
                continue;
            }
            ...
            var context = new ShapeBuildContext { Session = session, Parameters = parameters, CommandShapes = shapes, Fields = fields };
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    if (definition.OutputTopology != CommandTopologyKind.Any && (definition.OutputTopology & actualTopology) == 0)
                    {
                        if (session.Exists(transformed)) session.Delete(transformed);
                        result = ShapeBuildResult.Failed("OUTPUT_TOPOLOGY_INVALID", $"Command '{command.Name}' produced '{actualTopology}', expected '{definition.OutputTopology}'.");
                    }
                    else result = result with { Shape = transformed };
                }
            }
        ...
        var orderedStates = document.Commands.OrderBy(x => x.Order).ThenBy(x => x.Name, StringComparer.Ordinal)
            .Select(command => states.TryGetValue(command.Id, out var state) ? state : Failure(command, "COMMAND_NOT_PLANNED", $"Command '{command.Name}' could not be scheduled."))
            .ToArray();
        return new DocumentBuildResult(orderedStates.All(x => x.Status != CommandBuildStatus.Failed), shapes, orderedStates, DateTime.UtcNow - started);
    }
    ...
    public static CommandShapeBuilderRegistry CreateDefaultBuilderRegistry()
    {
        ...
        registry.Register(new EllipseCommandBuilder());
        registry.Register(new RegularPolygonCommandBuilder());
        registry.Register(new BezierCommandBuilder());
        registry.Register(new BSplineCommandBuilder());
        ...
        registry.Register(new ConeCommandBuilder());
        registry.Register(new SphereCommandBuilder());
        registry.Register(new TorusCommandBuilder());
        registry.Register(new WedgeCommandBuilder());
        registry.Register(new CompoundCommandBuilder());
        registry.Register(new SewCommandBuilder());
        registry.Register(new SolidFromShellCommandBuilder());
        ...
        registry.Register(new FilletCommandBuilder());
        registry.Register(new ChamferCommandBuilder());
        registry.Register(new OffsetCommandBuilder());
        registry.Register(new ShellCommandBuilder());
        ...
        registry.Register(new MoveCommandBuilder());
        registry.Register(new RotateShapeCommandBuilder());
        registry.Register(new ScaleShapeCommandBuilder());
        registry.Register(new MirrorCommandBuilder());
        return registry;
    }

    private static CommandBuildState Failure(ScriptCommand command, string code, string message) => new(command.Id, CommandBuildStatus.Failed, default, [ShapeBuildMessage.Error(code, message)]);
