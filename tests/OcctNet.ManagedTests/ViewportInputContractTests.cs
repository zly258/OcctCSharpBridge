...
public sealed class ViewportInputContractTests
{
    ...
    [TestMethod]
    public void DoubleClickIsAStablePointerInputKind()
    {
        var input = new OcctPointerInputEventArgs(
            OcctPointerInputKind.DoubleClicked,
            OcctPointerButton.Left,
            OcctPointerButtons.Left,
            12,
            24,
            0,
            OcctInputModifiers.None);

        Assert.AreEqual(OcctPointerInputKind.DoubleClicked, input.Kind);
        Assert.AreEqual(OcctPointerButton.Left, input.Button);
        Assert.AreEqual(4, (int)OcctPointerInputKind.DoubleClicked);
    }
