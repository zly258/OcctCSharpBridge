...
public sealed partial class MainForm
{
    private void BuildMenus()
    {
        ...
        {
            var menu = new ToolStripMenuItem(DemoLocalization.Text(definition.TextKey));
            AddUnifiedMenuItems(menu.DropDownItems, definition.Items);
        ...
        foreach (var definition in definitions)
        {
            switch (definition.Kind)
            {
                ...
                case DemoMenuItemKind.Command:
                {
                    ...
                    item.Click += (_, _) => RunCommand(commandId);
                    target.Add(item);
                    break;
                }
                ...
                {
                    var item = new ToolStripMenuItem(DemoLocalization.Text(RequiredTextKey(definition)));
                    AddUnifiedMenuItems(item.DropDownItems, definition.Children ?? Array.Empty<DemoMenuItemDefinition>());
                    target.Add(item);
                    break;
                }
                ...
                case DemoMenuItemKind.Action:
                {
                    ...
                    if (definition.CheckGroup != DemoMenuCheckGroup.None)
                    {
                        ...
                        item.Checked = IsMenuActionChecked(action);
                    }
