...
namespace
{
    ...
    TopoDS_Shape readDocToCompound(const Handle(TDocStd_Document)& doc)
    {
        ...
        {
            TopoDS_Shape shape = explorer.Current().Shape();
            if (!shape.IsNull())
            {
        ...
        if (count == 1)
        {
            explorer.Init(doc, XCAFPrs_DocumentExplorerFlags_OnlyLeafNodes);
            return explorer.Current().Shape();
        }
        return compound;
    }
}
...
extern "C"
{
        ...
        const OcctGltfExportOptions* options)
    {
        ...
        return exportShape(model, [&]
        {
            ...
            writer.SetTransformationFormat(options->transformToGltfCs != 0 ? RWGltf_WriterTrsfFormat_glTF : RWGltf_WriterTrsfFormat_Compact);
