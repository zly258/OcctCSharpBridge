...
The Windows installed path is the Binary SDK root itself. Portable SDK output is a separate publish artifact produced by Bridge `main`; it is not installed under `win-x64\\portable`. `OCCTCSHARPBRIDGE_SDK` overrides the Binary SDK root.
...
The unified package uses the installed Binary SDK plus the matching portable artifact generated by Bridge `main`. By default Demo looks for `artifacts/publish/OcctCSharpBridge-<version>-win-x64-portable`; use `-BridgePortableRoot` to override it. The default `all` layout also uses one shared private .NET 10 Desktop Runtime; `-SelfContained` and `-FrameworkDependent` remain explicit alternatives.
