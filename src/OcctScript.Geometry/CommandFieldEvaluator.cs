...
public sealed class CommandFieldEvaluator(ExpressionEngine expressionEngine)
{
        ...
        double? maximum = null)
    {
        ...
        if (!result.Success)
            throw new InvalidOperationException($"Command '{command.Name}' field '{fieldName}': {result.Error}");
        if (minimum.HasValue && result.Value < minimum.Value)
            throw new InvalidOperationException($"Command '{command.Name}' field '{fieldName}' must be at least {minimum.Value:G}.");
        if (maximum.HasValue && result.Value > maximum.Value)
            throw new InvalidOperationException($"Command '{command.Name}' field '{fieldName}' must be at most {maximum.Value:G}.");
        return result.Value;
    }
    ...
    public OcctPoint3d RequiredPoint(
        ScriptCommand command,
        string fieldName,
        IReadOnlyDictionary<string, double> parameters)
    {
    ...
    public OcctVector3d RequiredVector(
        ScriptCommand command,
        string fieldName,
        IReadOnlyDictionary<string, double> parameters,
        bool normalize = false)
    {
        ...
        if (vector.Length <= 1e-12)
            throw new InvalidOperationException($"Command '{command.Name}' field '{fieldName}' must not be a zero vector.");
        return normalize ? vector.Normalized() : vector;
    }

    public IReadOnlyList<OcctPoint3d> RequiredPoints(
        ScriptCommand command,
        string fieldName,
        IReadOnlyDictionary<string, double> parameters,
        int minimumCount = 2)
    {
        ...
        if (pointTokens.Length < minimumCount)
            throw new InvalidOperationException($"Command '{command.Name}' field '{fieldName}' requires at least {minimumCount} points.");
    ...
    private double[] EvaluateComponents(
        ScriptCommand command,
        string fieldName,
        int expectedCount,
        IReadOnlyDictionary<string, double> parameters) =>
        ...
        IReadOnlyDictionary<string, double> parameters)
    {
        ...
        if (parts.Length != expectedCount)
            throw new InvalidOperationException($"Command '{commandName}' field '{fieldName}' requires {expectedCount} comma-separated values.");
        ...
        for (var index = 0; index < parts.Length; index++)
        {
            ...
            if (!evaluated.Success)
                throw new InvalidOperationException($"Command '{commandName}' field '{fieldName}' component {index + 1}: {evaluated.Error}");
    ...
    private static string RequiredText(ScriptCommand command, string fieldName, bool preferExpression)
    {
        ...
        var value = preferExpression
            ? field.Expression.NullIfWhiteSpace() ?? field.Literal.NullIfWhiteSpace()
            : field.Literal.NullIfWhiteSpace() ?? field.Expression.NullIfWhiteSpace();
