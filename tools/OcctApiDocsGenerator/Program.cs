...
internal static partial class Program
{
    ...
    private sealed class XmlDocs
    {
        ...
        public string Method(Type type, MethodBase method)
        {
            ...
            var exact = _summaries.FirstOrDefault(pair =>
                pair.Key == prefix || pair.Key.StartsWith(prefix + "(", StringComparison.Ordinal));
    ...
    public static int Main(string[] args)
    {
        try
        {
            ...
            var dotnet = contractRoot.GetProperty("dotnet");
            var nativeAbi = contractRoot.GetProperty("nativeAbi");

            var targetFramework = dotnet.GetProperty("targetFramework").GetString()
                ?? throw new InvalidOperationException("dotnet.targetFramework is missing from bridge-contract.json.");
            var desktopTargetFramework = dotnet.GetProperty("desktopTargetFramework").GetString()
                ?? throw new InvalidOperationException("dotnet.desktopTargetFramework is missing from bridge-contract.json.");
            var bridgeVersion = contractRoot.GetProperty("bridgeVersion").GetString()
                ?? throw new InvalidOperationException("bridgeVersion is missing from bridge-contract.json.");
            var abiVersion = nativeAbi.GetProperty("current").GetInt32();
            var minimumAbiVersion = nativeAbi.GetProperty("minimumSupported").GetInt32();
            var apiPolicy = api.GetProperty("policy").GetString();

            if (abiVersion != 5 || minimumAbiVersion != 5 || !string.Equals(apiPolicy, "abi5-only", StringComparison.Ordinal))
                throw new InvalidOperationException("API documentation generation requires the Bridge 3 ABI5-only contract.");

            var inputs = DiscoverAssemblies(root, configuration, targetFramework, desktopTargetFramework);
            if (inputs.Length == 0)
                throw new InvalidOperationException("No public managed Bridge assemblies were discovered.");
            ...
            var avaloniaSmokeOutput = Path.Combine(
                root,
                "tests",
                "OcctNet.AvaloniaSmoke",
                "bin",
                "x64",
                configuration,
                targetFramework);
            ...
            var assemblyPaths = resolutionDirectories
            ...
            var loaded = inputs
                .Select(input => (
                    ...
                    Docs: new XmlDocs(input.XmlPath)))
                .ToArray();
            ...
            Console.WriteLine(
                $"Generated branch-aware API reference for {inputs.Length} assemblies, " +
                $"{publicTypeCount} public types and {functions.Length} Native exports.");
            return 0;
        }
    ...
    private static AssemblyInput[] DiscoverAssemblies(
        string root,
        ...
        string desktopTargetFramework)
    {
        ...
        return candidates
            .Where(candidate => File.Exists(Path.Combine(
                root,
                candidate.Directory.Replace('/', Path.DirectorySeparatorChar),
                candidate.Name + ".csproj")))
            .Select(candidate =>
            {
                ...
                return new AssemblyInput(
                    candidate.Name,
                    projectDirectory,
                    Path.Combine(output, candidate.Name + ".dll"),
                    Path.Combine(output, candidate.Name + ".xml"),
                    output);
            })
            .ToArray();
    }
    ...
    private static NativeHeader[] LoadNativeHeaders(string root, JsonElement api)
    {
        ...
        return values
            ...
            .Select(name =>
            {
                ...
                if (!File.Exists(path))
                    throw new FileNotFoundException($"Public Native header was not found: {name}", path);
    ...
    private static IEnumerable<NativeFunction> ParseNativeFunctions(NativeHeader header)
    {
        ...
        foreach (var rawLine in header.Content.Replace("\r\n", "\n", StringComparison.Ordinal).Split('\n'))
        {
            ...
            if (!match.Success)
                throw new InvalidOperationException(
                    $"Unable to parse Native declaration in {header.Name}: {normalized}");

            yield return new NativeFunction(
                header.Name,
        ...
        if (collecting)
            throw new InvalidOperationException($"Unterminated Native declaration in {header.Name}.");
    }
    ...
    private static void WriteLanguage(
        ...
        string language,
        bool chinese,
        (AssemblyInput Input, Assembly Assembly, XmlDocs Docs)[] loaded,
        NativeHeader[] headers,
        ...
        int abiVersion)
    {
        ...
        index.AppendLine(chinese
            ? "此目录由 `tools/OcctApiDocsGenerator` 根据当前分支实际存在的公开程序集自动生成。"
            : "This directory is generated by `tools/OcctApiDocsGenerator` from the public assemblies that actually exist on the current branch.").AppendLine();
        index.AppendLine($"- **Bridge:** `{bridgeVersion}`");
        index.AppendLine($"- **Native ABI:** `{abiVersion}`");
        index.AppendLine(
            $"- **{(chinese ? "公开程序集" : "Public assemblies")}:** " +
            string.Join(", ", loaded.Select(item => "`" + item.Input.Name + "`")));
        index.AppendLine(
            $"- **{(chinese ? "公开类型" : "Public types")}:** " +
            loaded.Sum(item => item.Assembly.GetExportedTypes().Length));
        index.AppendLine($"- **Native exports:** {functions.Length}").AppendLine();
        index.AppendLine(chinese
            ? "- [Native C ABI 完整参考](native-abi.md)"
            : "- [Complete Native C ABI Reference](native-abi.md)").AppendLine();
        ...
        foreach (var item in loaded)
        {
            ...
            foreach (var type in item.Assembly.GetExportedTypes().OrderBy(type => type.Namespace).ThenBy(type => type.Name))
            {
                ...
                File.WriteAllText(
                    Path.Combine(referenceRoot, fileName),
                    RenderType(item.Input.Name, type, item.Docs, chinese),
    ...
    private static string RenderType(string assemblyName, Type type, XmlDocs docs, bool chinese)
    {
        ...
        var description = docs.Type(type);
        if (!string.IsNullOrWhiteSpace(description)) builder.AppendLine(description).AppendLine();

        RenderMethods(
            builder,
            type,
            type.GetConstructors(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).Cast<MethodBase>(),
            docs,
            chinese ? "构造函数" : "Constructors",
            chinese);
        RenderProperties(
            builder,
            type,
        ...
        RenderEvents(
            builder,
            type,
            type.GetEvents(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly),
            docs,
            chinese);
        RenderMethods(
            builder,
        ...
        RenderFields(
            ...
            type.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly)
                .Where(field => type.IsEnum ? field.IsLiteral : !field.IsSpecialName),
            docs,
            chinese);
        ...
        bool chinese)
    {
        ...
        var values = methods.OrderBy(value => value.Name).ThenBy(Signature).ToArray();
        if (values.Length == 0)
        {
            builder.AppendLine(chinese ? "无。" : "None.").AppendLine();
            return;
        }

        foreach (var method in values)
        {
            builder.AppendLine("### `" + (method.IsConstructor ? type.Name : method.Name) + "`").AppendLine();
            var summary = docs.Method(type, method);
            if (!string.IsNullOrWhiteSpace(summary)) builder.AppendLine(summary).AppendLine();
            builder.AppendLine("```csharp").AppendLine(Signature(method)).AppendLine("```").AppendLine();
        }
    }
    ...
    private static void RenderProperties(
        ...
        bool chinese)
    {
        builder.AppendLine("## " + (chinese ? "属性" : "Properties")).AppendLine();
        var values = properties.OrderBy(value => value.Name).ToArray();
        if (values.Length == 0)
        {
            builder.AppendLine(chinese ? "无。" : "None.").AppendLine();
            return;
        ...
        {
            var access = value.CanRead && value.CanWrite ? "{ get; set; }" : value.CanRead ? "{ get; }" : "{ set; }";
            builder.AppendLine($"### `{value.Name}`").AppendLine();
            var summary = docs.Property(type, value);
            if (!string.IsNullOrWhiteSpace(summary)) builder.AppendLine(summary).AppendLine();
            builder.AppendLine("```csharp")
                .AppendLine($"public {DisplayType(value.PropertyType)} {value.Name} {access}")
                .AppendLine("```")
                .AppendLine();
        }
    }
    ...
    private static void RenderEvents(
        ...
        bool chinese)
    {
        builder.AppendLine("## " + (chinese ? "事件" : "Events")).AppendLine();
        var values = events.OrderBy(value => value.Name).ToArray();
        if (values.Length == 0)
        {
            builder.AppendLine(chinese ? "无。" : "None.").AppendLine();
            return;
        ...
        {
            builder.AppendLine($"### `{value.Name}`").AppendLine();
            var summary = docs.Event(type, value);
            if (!string.IsNullOrWhiteSpace(summary)) builder.AppendLine(summary).AppendLine();
            builder.AppendLine("```csharp")
                .AppendLine($"public event {DisplayType(value.EventHandlerType ?? typeof(Delegate))} {value.Name};")
                .AppendLine("```")
                .AppendLine();
        }
    }
    ...
    private static void RenderFields(
        ...
        bool chinese)
    {
        builder.AppendLine("## " + (chinese ? "字段 / 枚举值" : "Fields / Enum Values")).AppendLine();
        var values = fields.OrderBy(value => value.Name).ToArray();
        if (values.Length == 0)
        {
            builder.AppendLine(chinese ? "无。" : "None.").AppendLine();
            return;
        ...
        foreach (var value in values)
        {
            var literal = value.IsLiteral
                ? " = " + Convert.ToString(value.GetRawConstantValue(), System.Globalization.CultureInfo.InvariantCulture)
                : string.Empty;
            var summary = docs.Field(type, value);
            builder.AppendLine(
                $"- `{value.Name}` — `{DisplayType(value.FieldType)}`{literal}" +
    ...
    private static string Signature(MethodBase method)
    {
        var parameters = string.Join(
            ...
            method.GetParameters().Select(parameter =>
                $"{DisplayType(parameter.ParameterType.IsByRef ? parameter.ParameterType.GetElementType()! : parameter.ParameterType)} {parameter.Name}"));
        ...
        return $"public {modifier}{DisplayType(info.ReturnType)} {method.Name}({parameters})";
    }
    ...
    {
        var value = (type.FullName ?? type.Name).Replace('+', '.').Replace('`', '_');
