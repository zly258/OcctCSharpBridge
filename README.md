...
已删除原 `CadAvalonia` 项目。当前原生视口基于 Windows `HWND` 和 OCCT `WNT_Window`，Avalonia 的 `NativeControlHost` 只能承载平台原生控件，不能自动把该 Windows 视口转换为 Linux/macOS 实现。原示例因此不是真正的跨平台 CAD，且启动与原生句柄生命周期不稳定。

后续需要真正跨平台时，应分别实现 Windows、X11/Wayland 和 macOS 原生窗口适配，并使用各平台对应的 OCCT 二进制。
